...
| `disease` | 病害诊断 | 3 |
| `asr` | 语音识别（ffmpeg 转换 + DashScope ASR，Web 语音输入） | 3 |
| `auth` | 注册/登录/会话 | 2 |
...
| `stats` | 数据观测：概览、工具调用排行、趋势 | 1 |
| `gallery` | 图片中心：媒体资产记录、列表、删除 | 2 |
| `service` | 高德地图服务 | 1 |
| `tts` | 讯飞语音合成 | 1 |
| `utils` | JSON 工具 | 1 |
| `web` | 页面转发 | 1 |
...
| `src/main/resources/static/` | 前端 16 页面 + css/js |
...
- **工具调用埋点**：`ToolCallingService` 每次工具执行后写 `tool_call_logs`（user_id、工具名、成功/失败、耗时），失败不影响主流程
- **统计接口**：`/api/stats/overview`（用户/对话/工具调用/知识库等 9 项计数）、`/api/stats/tool-ranking`（Top10 + 成功率 + 均耗时）、`/api/stats/trend`（近 7 天对话量）
- **前端页面**：`/stats` 数据概览（统计卡 + 工具排行条形图 + 趋势柱状图），首页新增"数据概览"入口

### 4.6 Web 语音输入（2026-08-19 新增）

- **前端**：聊天页输入框旁麦克风按钮，`MediaRecorder` 录音（webm/opus），再次点击结束
- **后端**：`POST /api/asr/transcribe` 接收录音 → `AudioConverterService.convertAnyToWav16k16bitMono`（ffmpeg 通用转换，不依赖 AMR/SILK）→ `DashScopeAsrService` 识别（百炼 API Key）→ 返回文本
- **交互**：识别结果自动填入输入框，按 Enter 发送；浏览器不支持录音时自动隐藏按钮

### 4.7 图片中心（2026-08-19 新增）

- **资产来源**：AI 生图（`ImageGenerationService`）、TTS 音频（`TtsTool`）、上传分析图（`FileController`）三个产出点自动写入 `media_assets`
- **接口**：`GET /api/gallery/list?type=all|image|audio`、`DELETE /api/gallery/{id}`（校验归属后删文件+记录）
- **页面**：`/gallery` 图片网格 / 音频播放器 / 筛选 / 删除，首页新增“图片中心”入口
...
17 个静态页面，无构建工具，直接 fetch REST API：
...
- ✅ Web 语音输入已上线（2026-08-19）：`AsrController` + ffmpeg 通用转换 + DashScope ASR
