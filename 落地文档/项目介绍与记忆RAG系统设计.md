...
`PageController` 转发 15 个静态页面：`/`（落地页）、`/login`、`/register`、`/home`、`/chat`、`/care`、`/disease`、`/reminders`、`/briefing`、`/kb`、`/shop`、`/shop-publish`、`/community`、`/inventory`、`/timeline`。
...
`src/main/resources/static` 下 15 个静态页面，无前端构建工具，直接 fetch REST API：
