...
public class ILinkMessageListener {
    ...
    public ILinkMessageListener(MessageRouter messageRouter, FileStorageService fileStorageService, 
                                ...
                                ChatMemoryService chatMemoryService, ImageGenerationService imageGenerationService) {
                    ...
                    if (userId != null) {
                        if (reply.hasImage()) {
                            try {
                                ...
                                logger.error("Failed to send image, falling back to text", e);
        ...
        try {
            if (voiceContent != null) {
                ...
                if (recognizedText != null && !recognizedText.isEmpty()) {
                    ...
                    logger.info("Voice message converted to text, sending to AI router: {}", textContent);
                } else {
            ...
            if (textContent != null && imageContent != null) {
                logger.info("Received both text and image in same message, processing directly");
                var routeResult = messageRouter.routeTextMessage(textContent);
                logger.info("AI router determined intent: {} for content: {}", routeResult.getRouteType(), textContent);

                if (routeResult.getRouteType() == com.example.demo.router.MessageRouter.RouteType.IMAGE_GENERATION) {
                    String style = routeResult.getStyle();
                    logger.info("Image with generation request, style: {}", style);
                    String generatedImageUrl = messageRouter.processImageWithGeneration(imageContent, style);
                    byte[] generatedImageData = downloadImage(generatedImageUrl);
                    String extension = extractImageExtension(generatedImageUrl);
                    return new MessageReply(null, generatedImageData, extension, generatedImageUrl, routeResult.isNeedTts());
                }
                
                if (routeResult.getRouteType() == com.example.demo.router.MessageRouter.RouteType.IMAGE_EDIT) {
                    logger.info("Image with edit request, instruction: {}", textContent);
                    String imageBase64 = Base64.getEncoder().encodeToString(imageContent);
                    try {
                        String resultPath = imageGenerationService.editImage(imageBase64, textContent);
                        logger.info("Image edit result saved to: {}", resultPath);
                        byte[] resultData = downloadImage("file:///" + resultPath);
                        String extension = extractImageExtension(resultPath);
                        if (userId != null) {
                            userSessionService.storePendingImage(userId, imageBase64);
                        }
                        return new MessageReply(null, resultData, extension, resultPath, routeResult.isNeedTts());
                    } catch (Exception e) {
                        logger.error("Failed to edit image", e);
                        return new MessageReply("处理图片编辑时发生错误：" + e.getMessage());
                    }
                }
                
                String analysis = visionService.analyzeImageWithCustomPrompt(imageContent, textContent);
                if (userId != null) {
                    String imageBase64 = Base64.getEncoder().encodeToString(imageContent);
                    userSessionService.storePendingImage(userId, imageBase64);
                    chatMemoryService.saveMessagePair(userId, "[图片] " + textContent, analysis);
                }
                return new MessageReply("分析结果：\n" + analysis);
            }
            ...
            if (textContent != null) {
                ...
                if (routeResult.getRouteType() == com.example.demo.router.MessageRouter.RouteType.IMAGE_EDIT) {
                    if (userId != null && userSessionService.hasPendingImage(userId)) {
                        logger.info("Image edit request with pending image for user {}", userId);
                        String pendingImageBase64 = userSessionService.getPendingImageBase64(userId);
                        try {
                            String resultPath = imageGenerationService.editImage(pendingImageBase64, textContent);
                            logger.info("Image edit result saved to: {}", resultPath);
                            byte[] resultData = downloadImage("file:///" + resultPath);
                            String extension = extractImageExtension(resultPath);
                            return new MessageReply(null, resultData, extension, resultPath, routeResult.isNeedTts());
                        ...
                        return new MessageReply("请先发送一张图片，然后再发送编辑指令，比如“换成水彩风格”");
                    }
                }

                if (userId != null && userSessionService.hasPendingImage(userId)) {
                    logger.info("Text received with pending image for user {}, merging and processing", userId);
                    String pendingImageBase64 = userSessionService.getPendingImageBase64(userId);
                    
                    try {
                        String analysis = visionService.analyzeImageWithCustomPrompt("data:image/jpeg;base64," + pendingImageBase64, textContent);
                        if (userId != null) {
                            chatMemoryService.saveMessagePair(userId, "[图片] " + textContent, analysis);
                        }
                        return new MessageReply("分析结果：\n" + analysis);
                    } catch (Exception e) {
                        logger.error("Failed to process pending image with text", e);
                        userSessionService.clearPendingImage(userId);
                        return new MessageReply("处理图片时发生错误：" + e.getMessage());
                    }
                }

                logger.info("Processing text-only message with intent: {}", routeResult.getRouteType());
                if (routeResult.getRouteType() == com.example.demo.router.MessageRouter.RouteType.IMAGE_GENERATION) {
                    String result = messageRouter.process(routeResult);
                    logger.info("MessageRouter result: [{}]", result);
                    String imageUrl = result.replace("图片生成成功！链接：", "").trim();
                    logger.info("Extracted image URL: [{}]", imageUrl);
                    byte[] imageData = downloadImage(imageUrl);
                    String extension = extractImageExtension(imageUrl);
                    return new MessageReply(null, imageData, extension, imageUrl, routeResult.isNeedTts());
                }
                String reply = messageRouter.process(routeResult, userId);
                return new MessageReply(reply, routeResult.isNeedTts());
            }
            ...
            if (imageContent != null) {
                ...
                String imageBase64 = Base64.getEncoder().encodeToString(imageContent);
                userSessionService.storePendingImage(userId, imageBase64);
                return new MessageReply("图片已收到，请发送您的指令，比如“请提取图片中的文字”或“请总结图片内容”");
            }

            if (fileContent != null) {
                logger.info("Received file message, name: {}, size: {} bytes", fileName, fileContent.length);
                String question = textContent != null ? textContent.trim() : "总结这份文件的内容";
                String analysis = messageRouter.processFileQA(fileContent, fileName, question, userId);
                return new MessageReply("文件分析结果：\n" + analysis);
            }
    ...
    private void sendReplyWithTtsFallback(String userId, String text) {
        try {
            ...
            java.nio.file.Path tempPath = java.nio.file.Paths.get(System.getProperty("java.io.tmpdir"), fileName);
            java.nio.file.Files.write(tempPath, mp3Data);
            logger.info("Audio file saved to: {}", tempPath);
            
            byte[] verifyData = java.nio.file.Files.readAllBytes(tempPath);
            logger.info("Verified file size: {} bytes, first 4 bytes: 0x{} 0x{} 0x{} 0x{}", 
                verifyData.length,
                String.format("%02X", verifyData[0] & 0xFF),
                String.format("%02X", verifyData[1] & 0xFF),
                String.format("%02X", verifyData[2] & 0xFF),
                String.format("%02X", verifyData[3] & 0xFF));
